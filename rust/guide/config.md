...
**（0.4.1 起）** 进程启动时会在**运行目录**（启动 bot 的当前目录，非 `data_dir`）创建一个
以配置 QQ 号命名的 `<uin>.pid` 文件（如 `2717576556.pid`），并对它加 OS 级排他文件锁
（Windows `LockFileEx` / Unix `flock`），防止**同一个 QQ 被多个进程同时启动**——多开会导致
互相踢下线、并发写坏 `data/<uin>/icqq.db`、以及事件重复上报。

- 若该 QQ 已有一个实例在运行，新进程会在**登录前**立即报「QQ &lt;uin&gt; 已有一个实例在
  运行」并退出（不会连服务器）。不同 QQ 号各生成各自的 `<uin>.pid`、互不影响；同号才互斥。
- 锁随进程结束由操作系统自动释放（正常退出 / Ctrl+C / 崩溃都一样），**无需也不要**手动
  实现解锁。
- `<uin>.pid` 文件可能在 Ctrl+C / 崩溃退出后**残留**——这是正常的、无需手动清理：下次启动
  会重新加锁并覆盖它，残留文件不会被误判为"已在运行"。

::: tip
该文件无需配置、不入库（已在 `.gitignore` 忽略 `*.pid`）；纯运行期状态。
:::
